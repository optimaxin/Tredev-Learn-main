...
import { uploadSigned } from "@/lib/upload";
...
const MANTRA_AUDIO_MAX_BYTES = 2 * 1024 * 1024;
const AUDIO_ACCEPT = "audio/*";

const blank = () => ({ deity: "", title: "", devanagari: "", iast: "", meaning: "", audio_url: "" });

/** Staff-facing editor to add or edit a mantra under a deity (auto-links to the festival calendar). */
export default function MantraBuilder({ editing, onSaved, onCancelEdit }) {
  const [m, setM] = useState(editing || blank());
  const [saving, setSaving] = useState(false);
  const [audioPct, setAudioPct] = useState(null);
  const set = (p) => setM((s) => ({ ...s, ...p }));

  useEffect(() => { setM(editing || blank()); }, [editing]);

  const onAudio = async (file) => {
    if (!file) return;
    if (!file.type.startsWith("audio/")) return toast.error("Only audio files are allowed.");
    if (file.size > MANTRA_AUDIO_MAX_BYTES) return toast.error("Audio file must be 2MB or smaller.");
    setAudioPct(0);
    try {
      const { public_url } = await uploadSigned("/mantras/sign-upload", file, setAudioPct);
  ...
  return (
    <div className="rounded-2xl border border-border p-8 bg-card space-y-5" data-testid="mantra-builder">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-display font-bold text-2xl">{(m.id || m._id) ? "Edit mantra" : "Add a mantra"}</h3>
      </div>
      ...
      <div>
        ...
        <div className="mt-2 flex items-center gap-3 flex-wrap">
          <label className="inline-flex items-center gap-2 text-sm rounded-full border border-border px-4 h-10 cursor-pointer hover:bg-muted transition-colors">
            ...
            <input type="file" accept={AUDIO_ACCEPT} className="hidden" onChange={(e)=>onAudio(e.target.files?.[0])} data-testid="mantra-audio" />
          </label>
        ...
        <p className="text-[11px] text-muted-foreground mt-1">Audio only, 2MB max.</p>
      ...
      <div className="flex items-center gap-3">
        <Button onClick={save} disabled={saving} className="rounded-full h-11 px-8 bg-gradient-hot text-white border-0" data-testid="mantra-save">
          {saving ? "Saving…" : (m.id || m._id) ? "Save changes" : "Add mantra"}
        </Button>
        {(m.id || m._id) && (
          <Button type="button" variant="outline" className="rounded-full h-11 px-6" onClick={() => { setM(blank()); onCancelEdit && onCancelEdit(); }}>
            Cancel
          </Button>
        )}
      </div>
