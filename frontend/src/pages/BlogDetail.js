...
export default function BlogDetail() {
  ...
  return (
    <article className="max-w-3xl mx-auto py-16 px-6">
      ...
      <h1 className="font-display text-4xl md:text-6xl font-bold tracking-tight leading-[1.05]" data-testid="blog-title">{b.title}</h1>
            ...
            {related.map((r) => (
              <Link key={r.id} to={`/blog/${r.slug}`} className="group card-elevated rounded-xl border border-border overflow-hidden bg-card">
                ...
                <div className="p-4">
                  ...
                  <div className="font-display text-lg font-semibold mt-1 leading-tight">{r.title}</div>
                </div>
              </Link>
            ))}
          </div>
